...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        if (SecurityContextHolder.getContext().getAuthentication() == null) {
            ...
            //Bổ sung cấu hình cho phép Header và chấp nhận CROS policy ở đây
            response.setHeader("Access-Control-Allow-Origin", "*"); // Cho phép các domain khác gọi vào
            response.setHeader("Access-Control-Allow-Methods", "GET"); //Cho phép các phương thức
            response.setHeader("Access-Control-Allow-Headers", "Authorization, Content-Type, Accept, X-Requested-With, remember-me"); // Cho phép dữ liệu gửi lên server có dạng
            // json, form data
